...
for j=1:1024    % using the formula P(S)>= mt / N we calculate the m(row count)
        ...
        notzero=1;
%       chainlength=0;  % i was planning on storing the chainlenght for
...
%       sprintf('length of this chain is %d', chainlength);        
        hellman_table(j+1,2,:)=addparity(key56); % adding parity bits when an EP is found 
                                                 % and storing it in the table
end

display('hellmann table calculation is done');

    save('table'); % saving all the variables since creating the table takes about 1 hour
...
sorted_table=sort(hellman_table,2); % sort the table according to the EPs
	...
	while(~match) % when a match is found between EPs and the randkeyEP, it meaans 
        ...
        l=l+1; 
        match=isequal(sorted_table(l,2,:),randdp(:));
